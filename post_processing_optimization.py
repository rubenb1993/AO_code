...
for i in range(len(folder_names)):
    ...
    #flipped_x0 = nx_i - x0
    flipped_x0 = x0
    orig = np.array(PIL.Image.open(folder_name + "orig_scale.tif"))#np.ma.array(inter_0[flipped_y0-radius:flipped_y0+radius, flipped_x0-radius:flipped_x0+radius], mask = mask)
    ...
    inside_lsq = np.where(np.sqrt(x_pos_norm_lsq**2 + y_pos_norm_lsq**2) <= (1 + (box_len/vars_lsq[3])))
    ...
    inside_janss = np.where(np.sqrt(x_pos_norm_janss**2 + y_pos_norm_janss**2) <= (1 + (box_len/vars_janss[3])))
    x_pos_norm_janss, y_pos_norm_janss = mc.filter_positions(inside_lsq, x_pos_norm_janss, y_pos_norm_janss)
    ...
    f.savefig(fold_name+'methods_compared_norm.png', bbox_inches = 'tight', dpi = dpi_num)
    ...
    f2.savefig(fold_name+'zernike_coeff_compared_norm.png', bbox_inches = 'tight', dpi = dpi_num)
    ...
    np.save(folder_name + 'vars_dictionary_norm.npy', vars_dict)
    np.save(folder_name + 'coeff_dictionary_norm.npy', coeff_dict)
    json.dump(rms_dict, open(folder_name + "rms_dict_norm.txt", 'w'))
