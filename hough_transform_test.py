...
Id_int = np.zeros((2*radius, 2*radius, 4))
...
##for i in range(4):
##    Id_int[...,i] = Id_tot[y0-radius:y0+radius, x0-radius:x0+radius, i]
##    zeros_i = np.abs(Id_int[...,i]) <= 1
##    Id_zeros[zeros_i, i] = 1
...
##    delta_i[...,jj] = (tau_i[...,jj] + sigma[jj])/2.0
##    
##    sin_diff[...,jj] = np.sin(delta_i[...,jj])
##    Id_hat[..., jj] = Id_int[..., jj]/(-2.0 * sin_diff[...,jj])
...
titles = [r'a)', r'b)', r'c)', r'd)', r'e)', r'f)', r'g)', r'h)', r'i)', r'j)']
...
atanx = (Id_hat[..., Un_sol[1]] - np.cos(angfact) * Id_hat[..., Un_sol[0]]) / np.sin(angfact)

f, axarr = plt.subplots(1, len(Un_sol[0]), figsize = int_im_size)
...
## plot everything
for k in range(len(Un_sol[0])):
    im = axarr[k].imshow(np.ma.array(org_unwr[...,k], mask = mask), vmin = -10, vmax = 10)
...
axarr[0].imshow(np.ma.array(Im_i0[..., 0], mask = mask), cmap = 'bone', origin = 'lower')
...
fy = fx
f, axarr = plt.subplots(3,2, figsize = (0.88 * 4.98, 2 * 3.07))
...
axarr[0,0].imshow(np.ma.array(org_unwr[...,0], mask = mask), cmap = 'bone', vmin = -10, vmax = 15)
...
axarr[2,0].imshow(np.ma.array(np.real(filtered_phase), mask = mask), cmap = 'bone')
...
f, ax = plt.subplots(2,4,figsize = int_im_size)
ax[0,0].imshow(np.ma.array(org_phase[...,0], mask = mask), vmin = -np.pi, vmax = np.pi)
dx_org, dy_org = pw.delta_x(org_phase[...,0]), pw.delta_y(org_phase[...,0])
qual_map_org = pw.phase_derivative_var_map(dx_org, dy_org, 3)
ax[1,0].imshow(np.ma.array(qual_map_org, mask = mask), cmap = 'bone_r')
ax[0,0].set_title('original wrapped phase', fontsize = 7)
...
for i in range(3):
    ...
    qual_map_filt = pw.phase_derivative_var_map(dx_filt, dy_filt, 3)
    im = ax[0,i+1].imshow(np.ma.array(filter_phase, mask = mask), vmin = -np.pi, vmax = np.pi)
    ax[0,i+1].set_title(str(3+2*i) + ' x ' + str(3+2*i), fontsize = 7)
    k = i+1
    ax[1, i+1].imshow(np.ma.array(qual_map_filt, mask = mask), cmap = 'bone_r')
